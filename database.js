...
async function initDB() {
  ...
  if (fs.existsSync(DB_PATH)) {
    ...
    console.log('📦 Datenbank geladen aus:', DB_PATH);
  } else {
    db = new SQL.Database();
    console.log('📦 Neue Datenbank erstellt');
  }

  // Speichert DB bei Prozessende
...
// ─── MIGRATIONS (für bestehende DBs, fail-safe) ────────────────────────────
...
function activateSubscription(userId, stripeSubscriptionId, stripeCustomerId, paidUntil) {
  ...
  saveDB();
  console.log(`✅ Subscription aktiviert: User ${userId} bis ${paidUntil}`);
}
...
function cancelSubscription(stripeCustomerId) {
  db.run(
    `UPDATE users SET plan = 'cancelled', subscription_status = 'cancelled' WHERE stripe_customer_id = ?`,
    [stripeCustomerId]
  );
  saveDB();
}
...
function clearResetCode(userId) {
  db.run(`UPDATE users SET reset_code = NULL, reset_expires = NULL WHERE id = ?`, [userId]);
}

// ─── EMAIL ACCOUNTS ────────────────────────────────────────────────────────
function getEmailAccounts(userId) {
  const stmt = db.prepare(`SELECT * FROM email_accounts WHERE user_id = ? AND active = 1 ORDER BY id`);
  stmt.bind([userId]);
  const rows = [];
...
function deleteEmailAccount(accountId, userId) {
  db.run(`DELETE FROM email_accounts WHERE id = ? AND user_id = ?`, [accountId, userId]);
  saveDB();
}
...
// Alle aktiven Accounts über alle User — für den Cron-Job
function getAllActiveEmailAccounts() {
  const stmt = db.prepare(`SELECT * FROM email_accounts WHERE active = 1 ORDER BY user_id, id`);
  const rows = [];
  while (stmt.step()) rows.push(stmt.getAsObject());
  return rows;
}
...
function createLead(userId, accountId, data) {
  db.run(
    ...
    [
      userId, accountId,
      data.from_email || null, data.from_name || null,
      data.subject || null,    data.body || null,
      data.portal || 'Direkt', data.objekt_ref || null,
      data.score || 0,         data.kaufabsicht || null,
      data.finanzierung || null, data.zeitrahmen || null,
      data.zusammenfassung || null, data.antwort_entwurf || null,
...
function updateLeadWiedervorlage(id, userId, datum) {
  db.run(`UPDATE leads SET wiedervorlage = ? WHERE id = ? AND user_id = ?`, [datum || null, id, userId]);
  saveDB();
}
...
function getWiedervorlagen(userId) {
  ...
  const stmt = db.prepare(
    `SELECT * FROM leads WHERE user_id = ? AND wiedervorlage <= ? AND archived = 0 ORDER BY wiedervorlage`
  );
...
function getLeadStats(userId) {
  const all = db.exec(
    `SELECT
     ...
     FROM leads WHERE user_id = ${userId} AND archived = 0`
  );
  if (all.length && all[0].values.length) {
    const [gesamt, hoch, mittel, niedrig, beantwortet, heute] = all[0].values[0];
    return { gesamt: gesamt||0, hoch: hoch||0, mittel: mittel||0, niedrig: niedrig||0, beantwortet: beantwortet||0, heute: heute||0 };
  }
