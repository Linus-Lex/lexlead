...
const { notifyIfHot } = require('./notifications');
...
async function processMsg(raw, uid, acc) {
  try {
    ...
    if (analysis) {
      ...
      // SMS senden wenn heißer Lead
      const user = db.get('SELECT * FROM users WHERE id=?', [acc.user_id]);
      if (user?.phone) {
        await notifyIfHot({ ...lead, ...analysis }, user.phone);
      }
    } else {
