...
// ── Navigation ────────────────────────────────────────────
const titles = { all:'Alle Anfragen', hot:'Heiße Leads 🔥', new:'Neu & Ungelesen', settings:'E-Mail verbinden' };
...
function nav(view, btn) {
  ...
  document.getElementById('stats').style.display = isSettings ? 'none' : 'grid';
  document.getElementById('filter-row').style.display = isSettings ? 'none' : 'flex';
  document.getElementById('lead-list').style.display = isSettings ? 'none' : 'block';
  const sp = document.getElementById('settings-panel');
  sp.classList.toggle('show', isSettings);
