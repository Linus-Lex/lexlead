// server.js — LexLead v3.1
// Production-ready · Stripe · Trial-Sperrung · Outlook OAuth2 · iCloud · Landing · Legal
  ...
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px">
    <div style="width:100%;max-width:400px">
      ...
      <div class="card">
        ...
        ${req.query.sent?'<div class="alert alert-success">Code angefordert — sieh in den Render Logs nach deinem Reset-Code (🔑).</div>':''}
        ${req.query.error?`<div class="alert alert-error">${req.query.error}</div>`:''}
        ${!req.query.sent?`
        ...
        <form method="POST" action="/reset-password">
          <div class="form-group"><label>E-Mail</label><input type="email" name="email" placeholder="makler@beispiel.de" required></div>
          <div class="form-group"><label>Reset-Code (aus den Logs)</label><input type="text" name="code" placeholder="z.B. A3F9X2K1" required style="text-transform:uppercase"></div>
...
app.get('/auth/outlook/callback', auth, async (req, res) => {
  ...
  try {
    ...
    // Outlook über IMAP mit OAuth2-Token — Token als Passwort speichern
    db.addEmailAccount(
      req.session.userId,
      'Outlook – ' + email,
      email,
      ...
      <div style="border:1px solid var(--border2);border-radius:var(--radius-sm);padding:18px">
        ...
        <div class="form-group">
          ...
          <select onchange="fillHost(this.value)">
            ...
            <option value="outlook.office365.com:993:outlook">Outlook (App-Passwort)</option>
            <option value="custom:993:standard">Eigener Server</option>
          </select>
        </div>

        <!-- Dynamischer Hinweis je nach Anbieter -->
...
async function start() {
  ...
  app.listen(PORT, () => {
    console.log(`🚀 LexLead v3.1 auf Port ${PORT}`);
