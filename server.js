// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  LexLead v3.0 — server.js                                               ║
// ║  Production-ready · Stripe · Trial-Sperrung · Landing Page · Legal      ║
// ╚══════════════════════════════════════════════════════════════════════════╝
...
// ─── WEBHOOK ROUTE (vor express.json — Stripe braucht raw body) ───────────
...
// Trial / Subscription Guard — prüft ob Account noch aktiv ist
function subscriptionGuard(req, res, next) {
  if (!req.session.userId) return res.redirect('/login');
  const user = db.getUserById(req.session.userId);
  if (!user) return res.redirect('/login');

  // Paid-User — prüfe ob paid_until noch gültig
  if (user.plan === 'paid') {
  ...
  // Trial-User — prüfe ob 30 Tage abgelaufen
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --accent:   #3d7ef6;
      --accent2:  #5b8ff8;
      --accent-glow: rgba(61,126,246,0.18);
      --green:    #10b981;
      --yellow:   #f59e0b;
      --red:      #ef4444;
      --orange:   #f97316;
    ...
    .btn-primary {
      ...
      box-shadow: 0 2px 12px rgba(61,126,246,0.35);
    }
    .btn-primary:hover { background: var(--accent2); box-shadow: 0 4px 20px rgba(61,126,246,0.5); transform: translateY(-1px); }
    ...
    .btn-upgrade {
      background: linear-gradient(135deg, #3d7ef6, #8b5cf6);
      color: #fff; box-shadow: 0 2px 16px rgba(139,92,246,0.4);
    }
    ...
    .nav-brand .logo-mark {
      ...
      box-shadow: 0 2px 10px rgba(61,126,246,0.45);
    }
    .nav-link {
    ...
    .trial-pill {
      ...
      color: var(--yellow); font-size: 0.76rem; font-weight: 700;
      font-family: var(--font-display);
    }
    ...
    .cal-cell {
      background: var(--bg2); border: 1px solid var(--border);
      border-radius: 10px; padding: 9px; min-height: 88px;
    ...
    .cal-event  {
      font-size: 0.7rem; padding: 3px 6px; border-radius: 5px;
...
// Toast JS snippet (injected into every page)
const toastScript = `
    ...
    <div class="nav-brand">
      <div class="logo-mark">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M3 12L8 4L13 12" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M5.5 9.5H10.5" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
        </svg>
      </div>
      LexLead
    </div>
...
<head>
  ...
  <title>LexLead – KI-Filter für Immobilien-Anfragen</title>
  ${baseStyles}
  <style>
    .hero {
    ...
    .hero::before {
      ...
      background:
        radial-gradient(ellipse 80% 50% at 50% -10%, rgba(61,126,246,0.18) 0%, transparent 60%),
    ...
    .hero-badge {
      ...
      background: rgba(61,126,246,0.1); border: 1px solid rgba(61,126,246,0.25);
    ...
    .hero p {
      font-size: 1.1rem; color: var(--muted2); max-width: 540px;
      margin-bottom: 36px; line-height: 1.7;
      animation: fadeUp 0.8s ease;
    }
    .hero-cta { display: flex; gap: 12px; justify-content: center; flex-wrap: wrap; animation: fadeUp 0.9s ease; }
    .features {
      display: grid; grid-template-columns: repeat(3,1fr);
      gap: 16px; max-width: 1000px; margin: 80px auto 0;
      padding: 0 20px; animation: fadeUp 1s ease;
    }
    .feature-card {
      background: var(--bg2); border: 1px solid var(--border);
    ...
    .feature-icon {
      ...
      background: var(--accent-glow); border: 1px solid rgba(61,126,246,0.2);
      display: flex; align-items: center; justify-content: center;
    ...
    .pricing-section {
      max-width: 460px; margin: 80px auto 0; padding: 0 20px;
      text-align: center;
    }
    .pricing-card {
      background: var(--bg2); border: 1px solid var(--border2);
    ...
    .pricing-card::before {
      ...
      background: linear-gradient(135deg, rgba(61,126,246,0.06), rgba(139,92,246,0.06));
      pointer-events: none;
    }
    .price-tag {
    ...
    .land-nav {
      ...
      background: rgba(8,10,15,0.8); backdrop-filter: blur(16px);
      border-bottom: 1px solid var(--border);
    ...
    .land-nav-brand {
      ...
      font-size: 1.1rem; display: flex; align-items: center; gap: 9px;
    }
    .land-nav-right { margin-left: auto; display: flex; gap: 10px; align-items: center; }
    footer {
      text-align: center; padding: 40px 20px;
      color: var(--muted); font-size: 0.82rem; margin-top: 80px;
...
<body>
  <nav class="land-nav">
    <div class="land-nav-brand">
      <div class="logo-mark" style="width:30px;height:30px;border-radius:8px;background:var(--accent);display:flex;align-items:center;justify-content:center;box-shadow:0 2px 10px rgba(61,126,246,0.45)">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M3 12L8 4L13 12" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M5.5 9.5H10.5" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
        </svg>
  ...
  <section class="features" id="features">
    <div class="feature-card">
      ...
      <h3>KI-Scoring 1–10</h3>
      <p>Claude analysiert Kaufabsicht, Finanzierung und Zeitrahmen jeder Anfrage automatisch.</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon">✉</div>
      <h3>Alle Portale</h3>
      <p>ImmoScout24, Immowelt, Kleinanzeigen, Gmail, Outlook — ein Postfach für alles.</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon">⚡</div>
      <h3>Antwort in 1 Klick</h3>
      <p>Fertige Antwort-Entwürfe per KI. Kopieren, anpassen, senden — fertig.</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon">📅</div>
      <h3>Kalender & Termine</h3>
      <p>Besichtigungen, Anrufe, Fristen — direkt aus dem Lead heraus erstellen.</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon">🔔</div>
      <h3>Wiedervorlage</h3>
      <p>Kein Lead geht vergessen. Wiedervorlage setzen, Erinnerung erhalten.</p>
    </div>
    <div class="feature-card">
      <div class="feature-icon">📊</div>
      <h3>Dashboard</h3>
      <p>Überblick über alle Leads, Scores und Konversionsraten auf einen Blick.</p>
    </div>
  </section>
  ...
  <section class="pricing-section">
    ...
    <div class="pricing-card">
      ...
      <ul class="price-features">
        <li>Unbegrenzte Leads & Postfächer</li>
        <li>KI-Analyse mit Claude Sonnet</li>
        <li>Automatischer E-Mail-Abruf</li>
        <li>Kalender & Wiedervorlage</li>
        <li>Antwort-Entwürfe per KI</li>
        <li>Alle Portal-Erkennungen</li>
      </ul>
      <a href="/login?tab=register" class="btn btn-upgrade btn-lg" style="width:100%;justify-content:center">
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Impressum, AGB & Datenschutz – LexLead</title>
  ${baseStyles}
  <style>
    ...
    h3 { font-family:var(--font-display); font-size:0.9rem; font-weight:700; margin:16px 0 6px; color:var(--text); }
...
<body>
  ...
  <div class="page" style="padding:40px 20px 80px">
    ...
    <div id="impressum" class="pane active">
      ...
      <h2>Hinweis zur Umsatzsteuer</h2>
      <p>Patrick Rümmler ist Kleinunternehmer im Sinne von § 19 UStG. Es wird daher keine Umsatzsteuer berechnet und ausgewiesen.</p>
      <h2>Streitschlichtung</h2>
      <p>Die EU-Kommission stellt eine Plattform zur Online-Streitbeilegung bereit: <a href="https://ec.europa.eu/consumers/odr" target="_blank">ec.europa.eu/consumers/odr</a>. Wir nehmen nicht an Streitbeilegungsverfahren vor einer Verbraucherschlichtungsstelle teil.</p>
      <h2>Haftung für Inhalte</h2>
      <p>Als Diensteanbieter sind wir gemäß § 7 Abs. 1 TMG für eigene Inhalte verantwortlich. Nach §§ 8–10 TMG sind wir nicht verpflichtet, übermittelte oder gespeicherte fremde Informationen zu überwachen.</p>
      <h2>Urheberrecht</h2>
      <p>Die durch den Seitenbetreiber erstellten Inhalte unterliegen dem deutschen Urheberrecht. Vervielfältigung und Verwertung bedürfen der schriftlichen Zustimmung.</p>
    </div>
    ...
    <div id="agb" class="pane">
      ...
      <p>Diese Allgemeinen Geschäftsbedingungen (AGB) gelten für alle Verträge zwischen Patrick Rümmler, Ossietzkystraße 11C, 13187 Berlin (nachfolgend „Anbieter") und den Nutzern der SaaS-Plattform LexLead (nachfolgend „Kunde"). Abweichende Bedingungen des Kunden werden nicht anerkannt, sofern der Anbieter diesen nicht ausdrücklich schriftlich zugestimmt hat.</p>

      <h2>§ 2 Leistungsbeschreibung</h2>
      <p>LexLead ist eine webbasierte Software (SaaS), die Immobilienmaklern die automatisierte Analyse und Bewertung eingehender E-Mail-Anfragen mittels KI-Technologie ermöglicht. Der Funktionsumfang umfasst insbesondere:</p>
      <ul>
        <li>KI-gestützte Bewertung eingehender Leads mit Score 1–10</li>
        <li>Automatischer Abruf verbundener E-Mail-Postfächer via IMAP</li>
        <li>Erstellung von Antwort-Entwürfen durch KI</li>
        <li>Kalender- und Wiedervorlagefunktion</li>
        <li>Dashboard mit Statistiken</li>
      </ul>
      <p>Der Anbieter behält sich vor, den Funktionsumfang jederzeit zu erweitern oder anzupassen. Wesentliche Leistungseinschränkungen werden dem Kunden mit einer Frist von 30 Tagen angekündigt.</p>

      <h2>§ 3 Vertragsschluss</h2>
      <p>Der Vertrag kommt zustande, wenn der Kunde sich auf der Plattform registriert und nach Ablauf der kostenlosen Testphase ein kostenpflichtiges Abonnement abschließt. Mit der Registrierung erklärt der Kunde, diese AGB gelesen und akzeptiert zu haben.</p>

      <h2>§ 4 Testphase</h2>
      <p>Neukunden erhalten eine kostenlose Testphase von 30 Tagen ab Registrierung. Während der Testphase ist der volle Funktionsumfang nutzbar. Nach Ablauf der Testphase wird der Zugang gesperrt bis ein kostenpflichtiges Abonnement abgeschlossen wird. Es besteht kein Anspruch auf Verlängerung der Testphase.</p>

      <h2>§ 5 Preise und Zahlung</h2>
      <p>Der monatliche Abonnementpreis beträgt <strong>149,00 Euro</strong> pro Maklerbüro (Einzellizenz). Der Anbieter ist Kleinunternehmer gemäß § 19 UStG — es wird keine Umsatzsteuer ausgewiesen.</p>
      <p>Die Zahlung erfolgt monatlich im Voraus per Kreditkarte oder SEPA-Lastschrift über den Zahlungsdienstleister Stripe. Der erste Betrag wird unmittelbar nach Abschluss des Abonnements fällig. Folgebeträge werden automatisch monatlich zum gleichen Datum abgebucht.</p>
      <p>Bei fehlgeschlagener Zahlung behält sich der Anbieter vor, den Zugang zur Plattform nach einer Kulanzfrist von 7 Tagen zu sperren.</p>

      <h2>§ 6 Laufzeit und Kündigung</h2>
      <p>Das Abonnement läuft auf unbestimmte Zeit und verlängert sich automatisch monatlich. Der Kunde kann das Abonnement jederzeit zum Ende des laufenden Kalendermonats kündigen. Die Kündigung erfolgt über das Kundenportal von Stripe oder per E-Mail an <a href="mailto:lpruemmler@gmail.com">lpruemmler@gmail.com</a>.</p>
      <p>Nach Eingang der Kündigung bleibt der Zugang bis zum Ende des bereits bezahlten Zeitraums bestehen. Eine anteilige Erstattung bereits gezahlter Beträge erfolgt nicht.</p>
      <p>Das Recht zur außerordentlichen Kündigung aus wichtigem Grund bleibt unberührt. Ein wichtiger Grund auf Seiten des Anbieters liegt insbesondere vor, wenn der Kunde gegen diese AGB verstößt oder ausstehende Zahlungen trotz Mahnung nicht begleicht.</p>

      <h2>§ 7 Verfügbarkeit und Wartung</h2>
      <p>Der Anbieter bemüht sich um eine Verfügbarkeit der Plattform von 99 % im Jahresmittel, ausgenommen geplante Wartungsfenster. Ein Rechtsanspruch auf eine bestimmte Verfügbarkeit besteht nicht. Geplante Wartungsarbeiten werden nach Möglichkeit im Voraus angekündigt.</p>
      ...
      <ul>
        ...
        <li>Keine automatisierten Zugriffe oder Scraping-Versuche durchzuführen</li>
        <li>Den Anbieter unverzüglich über unbefugte Nutzung seines Accounts zu informieren</li>
        <li>Sicherzustellen, dass die Verarbeitung der über IMAP abgerufenen E-Mail-Daten mit der DSGVO vereinbar ist</li>
      </ul>

      <h2>§ 9 Datenschutz und Vertraulichkeit</h2>
      <p>Die Verarbeitung personenbezogener Daten erfolgt gemäß der Datenschutzerklärung des Anbieters, die unter <a href="/legal">/legal</a> abrufbar ist. Der Anbieter behandelt alle vom Kunden übermittelten Daten vertraulich und gibt diese nicht ohne Zustimmung des Kunden an Dritte weiter, soweit dies nicht zur Leistungserbringung erforderlich ist.</p>
      <p>Der Kunde ist selbst verantwortlich dafür, dass die Verarbeitung von E-Mail-Inhalten durch LexLead im Einklang mit den datenschutzrechtlichen Anforderungen seines Unternehmens steht.</p>
      ...
      <p>Für leicht fahrlässig verursachte Schäden haftet der Anbieter nur bei Verletzung wesentlicher Vertragspflichten (Kardinalpflichten), und zwar begrenzt auf den vorhersehbaren, vertragstypischen Schaden. Die Haftung für leicht fahrlässige Pflichtverletzungen ist auf den 3-fachen Monatsbeitrag des Kunden begrenzt.</p>
      <p>Die KI-generierten Analysen und Antwort-Entwürfe sind Empfehlungen und ersetzen keine professionelle Beratung. Der Anbieter übernimmt keine Haftung für Entscheidungen, die der Kunde auf Basis der KI-Ausgaben trifft.</p>

      <h2>§ 11 Änderungen der AGB</h2>
      <p>Der Anbieter behält sich vor, diese AGB mit einer Ankündigungsfrist von 30 Tagen zu ändern. Die Änderungen werden dem Kunden per E-Mail mitgeteilt. Widerspricht der Kunde nicht innerhalb von 30 Tagen nach Zugang der Mitteilung, gelten die neuen AGB als akzeptiert. Auf dieses Widerspruchsrecht wird in der Änderungsmitteilung ausdrücklich hingewiesen.</p>

      <h2>§ 12 Schlussbestimmungen</h2>
      <p>Es gilt das Recht der Bundesrepublik Deutschland unter Ausschluss des UN-Kaufrechts. Gerichtsstand für alle Streitigkeiten aus diesem Vertrag ist Berlin, sofern der Kunde Kaufmann, juristische Person des öffentlichen Rechts oder öffentlich-rechtliches Sondervermögen ist.</p>
    ...
    <div id="datenschutz" class="pane">
      ...
      <p>Patrick Rümmler, Ossietzkystraße 11C, 13187 Berlin<br>E-Mail: <a href="mailto:lpruemmler@gmail.com">lpruemmler@gmail.com</a></p>
      <h2>2. Erhobene Daten &amp; Zweck</h2>
      <ul>
        <li><strong>Registrierungsdaten:</strong> Name, E-Mail, Firmenname — zur Bereitstellung des Dienstes</li>
        <li><strong>E-Mail-Zugangsdaten (IMAP):</strong> verschlüsselt gespeichert, ausschließlich zum Abruf eingehender Nachrichten</li>
        <li><strong>Lead-Daten:</strong> Inhalte eingehender E-Mails zur KI-Analyse via Anthropic API</li>
        <li><strong>Zahlungsdaten:</strong> ausschließlich durch Stripe verarbeitet — keine Kreditkartendaten bei uns</li>
        <li><strong>Nutzungsdaten:</strong> Log- und Session-Daten zur technischen Bereitstellung</li>
      </ul>
      <h2>3. Rechtsgrundlage</h2>
      <p>Art. 6 Abs. 1 lit. b DSGVO (Vertragserfüllung) sowie Art. 6 Abs. 1 lit. f DSGVO (berechtigtes Interesse).</p>
      <h2>4. Weitergabe an Dritte</h2>
      <ul>
        <li><strong>Anthropic, Inc. (USA)</strong> — KI-Analyse. Grundlage: Standardvertragsklauseln (SCC).</li>
        <li><strong>Stripe, Inc. (USA)</strong> — Zahlungsabwicklung, PCI-DSS-zertifiziert. Grundlage: SCC.</li>
        <li><strong>Render Services, Inc. (USA)</strong> — Hosting. Grundlage: SCC.</li>
      </ul>
      <h2>5. Speicherdauer</h2>
      <p>Daten werden gespeichert solange das Konto aktiv ist. Nach Kündigung Löschung binnen 30 Tagen, außer gesetzliche Aufbewahrungspflichten (Rechnungsdaten: 10 Jahre §147 AO).</p>
      ...
      <p>Kontakt: <a href="mailto:lpruemmler@gmail.com">lpruemmler@gmail.com</a></p>
      <h2>7. Beschwerderecht</h2>
      <p>Berliner Beauftragte für Datenschutz und Informationsfreiheit, Friedrichstr. 219, 10969 Berlin · <a href="https://www.datenschutz-berlin.de" target="_blank">datenschutz-berlin.de</a></p>
      <h2>8. Cookies &amp; Tracking</h2>
      <p>Ausschließlich technisch notwendige Session-Cookies. Kein Tracking, keine Analytics, keine Werbecookies.</p>
      <h2>9. SSL-Verschlüsselung</h2>
      <p>Diese Seite nutzt TLS-Verschlüsselung. Erkennbar am „https://" in der Adresszeile.</p>
    </div>
  </div>
  <script>
    ...
    // AGB direkt anzeigen wenn URL #agb enthält
...
app.get('/upgrade', auth, (req, res) => {
  ...
  const messages = {
    trial:    { title: 'Dein Trial ist abgelaufen', sub: '30 Tage kostenlos — deine Zeit ist um. Upgrade um weiterzumachen.' },
    expired:  { title: 'Abo abgelaufen', sub: 'Dein Abo ist ausgelaufen. Bitte erneuere es.' },
    cancelled:{ title: 'Abo gekündigt', sub: 'Dein Abo wurde beendet. Du kannst es jederzeit reaktivieren.' },
  };
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Upgrade – LexLead</title>
  ${baseStyles}
</head>
<body>
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;
    background:radial-gradient(ellipse 70% 50% at 50% 0%, rgba(61,126,246,0.12) 0%, transparent 60%)">
    <div style="width:100%;max-width:460px;text-align:center">
      ...
      <div class="card" style="text-align:left;margin-bottom:20px">
        <div style="font-family:var(--font-display);font-size:2.5rem;font-weight:800;text-align:center;margin-bottom:4px">
          €149<span style="font-size:1rem;font-weight:500;color:var(--muted)">/Monat</span>
        </div>
        ...
        <ul style="list-style:none;margin-bottom:20px">
          ${['Unbegrenzte Leads','KI-Scoring mit Claude','Alle Postfächer','Kalender & Wiedervorlage','Antwort-Entwürfe'].map(f =>
            `<li style="padding:8px 0;border-bottom:1px solid var(--border);font-size:0.88rem;display:flex;align-items:center;gap:10px">
              <span style="color:var(--green);font-weight:800">✓</span> ${f}
            </li>`).join('')}
        </ul>
        <button onclick="startCheckout()" id="upgradeBtn" class="btn btn-upgrade btn-lg" style="width:100%;justify-content:center">
          Jetzt für 149€/Monat freischalten
        </button>
      </div>
    ...
    async function startCheckout() {
      ...
      try {
        ...
        if (d.ok && d.url) { window.location.href = d.url; }
          ...
          showToast(d.error || 'Fehler beim Checkout', 'error');
          btn.disabled = false; btn.textContent = 'Jetzt für 149€/Monat freischalten';
        }
      } catch {
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Login – LexLead</title>
  ${baseStyles}
  <style>
    .auth-page { min-height:100vh; display:flex; align-items:center; justify-content:center; padding:20px;
      background:radial-gradient(ellipse 60% 50% at 50% -10%, rgba(61,126,246,0.15) 0%, transparent 55%); }
    ...
    .auth-tab { flex:1; padding:9px; text-align:center; border-radius:var(--radius-sm); cursor:pointer;
      font-weight:700; font-size:0.85rem; border:1px solid var(--border2); color:var(--muted);
      background:transparent; font-family:var(--font-display); transition:all 0.15s; }
...
<body>
  <div class="auth-page">
    <div class="auth-box">
      ...
      <div class="auth-card">
        ...
        <div id="login" class="pane active">
          ...
          <div style="text-align:center;margin-top:14px">
            <a href="/forgot-password" style="color:var(--muted);font-size:0.82rem">Passwort vergessen?</a>
          </div>
        </div>
      ...
      <p style="text-align:center;color:var(--muted);font-size:0.76rem;margin-top:14px">
        149€/Monat nach Trial · Kein Risiko · <a href="/legal" style="color:var(--muted)">Impressum</a>
      </p>
    </div>
  </div>
  <script>
...
app.get('/forgot-password', (req, res) => {
  res.send(`<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8"><title>Passwort vergessen</title>${baseStyles}</head><body>
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px">
    <div style="width:100%;max-width:380px">
      ...
      <div class="card">
        ...
        ${req.query.sent ? '<div class="alert alert-success">Falls die E-Mail existiert, sieh in den Render Logs nach dem Reset-Code.</div>' : ''}
...
app.get('/dashboard', subscriptionGuard, (req, res) => {
  ...
  const statusBadge = s => {
    const m={neu:['badge-blue','Neu'],beantwortet:['badge-green','Beantwortet'],abgelehnt:['badge-red','Abgelehnt'],warten:['badge-yellow','Warten']};
...
<title>Dashboard – LexLead</title>${baseStyles}</head><body>
...
<div class="page">
  ...
  <div style="background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.18);border-radius:var(--radius);padding:16px 20px;margin-bottom:20px">
    <div style="font-weight:700;color:var(--yellow);margin-bottom:10px;font-family:var(--font-display)">
      ⏰ ${wiedervorlagen.length} Wiedervorlage${wiedervorlagen.length>1?'n':''} heute fällig
    </div>
  ...
  <div class="grid-4 mb-4">
    ...
    <div class="stat-card" style="--accent-line:var(--yellow)">
  ...
  <div class="grid-2">
        ...
        <tbody>
          ...
          <tr style="cursor:pointer" onclick="location.href='/leads/${l.id}'">
            ...
            <td>
              ...
              <div class="text-sm text-muted" style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${l.subject}</div>
            </td>
    ...
    <div class="card">
      ...
      ${stats.gesamt>0?`
      ${[['Hoch (≥7)','hoch','var(--green)'],['Mittel (4–6)','mittel','var(--yellow)'],['Niedrig (1–3)','niedrig','var(--red)']].map(([label,key,color])=>`
      <div style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;margin-bottom:5px">
          <span class="text-sm">${label}</span>
          <span class="text-sm" style="color:${color}">${stats[key]}</span>
        </div>
...
async function checkNow(btn) {
  ...
  try {
    ...
    showToast((d.neue||0)+' neue Leads abgerufen');
...
app.get('/leads', subscriptionGuard, (req, res) => {
  ...
  const leads = db.getLeads(req.session.userId, {
    status: status||null, portal: portal||null,
    search: search||null, minScore: min?parseInt(min):null
  });
  ...
  const kaufBadge = k=>{
    const m={Hoch:'badge-green',Mittel:'badge-yellow',Niedrig:'badge-red'};
    return k?`<span class="badge ${m[k]||'badge-gray'}">${k}</span>`:'<span class="text-muted">–</span>';
  };
  const timeAgo = dt=>{
    if(!dt)return'';
  ...
  res.send(`<!DOCTYPE html>
<html lang="de"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Leads – LexLead</title>${baseStyles}</head><body>
      ...
      <tbody>
        ...
        <tr style="cursor:pointer" onclick="location.href='/leads/${l.id}'">
          ...
          <td style="max-width:200px">
            <div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${l.subject}</div>
            ${l.wiedervorlage?`<div class="text-xs" style="color:var(--yellow)">⏰ ${l.wiedervorlage}</div>`:''}
          </td>
          <td><span class="badge badge-blue">${l.portal}</span></td>
...
app.get('/leads/:id', subscriptionGuard, (req, res) => {
  ...
  res.send(`<!DOCTYPE html>
<html lang="de"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${lead.from_name||lead.from_email} – LexLead</title>${baseStyles}</head><body>
${navbar('leads', user, req.trialDaysLeft)}
<div class="page">
...
<script>
  ...
  async function updateStatus(val) {
    await fetch('/api/leads/'+leadId+'/status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:val})});
    showToast('Status aktualisiert');
  }
  async function saveNotiz() {
    await fetch('/api/leads/'+leadId+'/notiz',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({notiz:document.getElementById('notizText').value})});
    showToast('Notiz gespeichert');
  }
  async function saveWiedervorlage() {
    const datum=document.getElementById('wiedervorlageDate').value;
    await fetch('/api/leads/'+leadId+'/wiedervorlage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({datum})});
...
app.get('/kalender', subscriptionGuard, (req, res) => {
  ...
  for(let d=1;d<=tageImMonat;d++){
    ...
    cells+=`<div class="cal-cell ${isToday?'cal-today':''}">
      ...
      <div class="cal-event" style="background:${typeColor[t.typ]||'var(--accent)'}18;border-left:2px solid ${typeColor[t.typ]||'var(--accent)'}" title="${t.titel}">
        ${typeIcon[t.typ]||'📌'} ${t.uhrzeit?t.uhrzeit+' ':''}${t.titel.substring(0,18)}
      </div>`).join('')}
  ...
  res.send(`<!DOCTYPE html>
<html lang="de"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Kalender – LexLead</title>${baseStyles}</head><body>
...
<div class="modal-overlay" id="terminModal" onclick="if(event.target===this)this.classList.remove('open')">
  <div class="modal">
    <div class="modal-header">Neuer Termin
...
<script>
  async function createTerminFromModal(){
    ...
    await fetch('/api/termine',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
      titel,typ:document.getElementById('tTyp').value,datum,
...
app.get('/einstellungen', subscriptionGuard, (req, res) => {
  ...
  res.send(`<!DOCTYPE html>
<html lang="de"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Einstellungen – LexLead</title>${baseStyles}</head><body>
...
<div class="page" style="max-width:740px">
  ...
  <div class="card mb-4">
    ...
    <div style="margin-bottom:16px">
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--border)">
        ...
        <div style="flex:1">
          ...
          ${a.last_check?`<div class="text-xs text-muted">Zuletzt: ${new Date(a.last_check).toLocaleString('de-DE')}</div>`:''}
        </div>
    ...
    </div>`:`<p class="text-muted text-sm" style="margin-bottom:16px">Noch kein Postfach verbunden.</p>`}

    <div style="border:1px solid var(--border2);border-radius:var(--radius-sm);padding:18px">
      <div style="font-weight:700;font-family:var(--font-display);margin-bottom:14px">Neues Postfach</div>
      <div class="form-group">
    ...
    <div class="mt-4" style="background:var(--bg3);border-radius:var(--radius-sm);padding:12px">
      <div style="font-size:0.76rem;font-weight:700;color:var(--muted);margin-bottom:5px;font-family:var(--font-display)">💡 Gmail: App-Passwort verwenden</div>
      <div class="text-sm text-muted">Google-Konto → Sicherheit → 2FA → App-Passwörter. Normales Passwort funktioniert nicht.</div>
    </div>
  </div>
  ...
  <div class="card mb-4">
    ...
    <div style="display:flex;align-items:center;gap:12px">
      <div>
        <div class="text-sm text-muted">API Key Status</div>
        <div style="margin-top:4px">
          ${process.env.ANTHROPIC_API_KEY
            ? `<span class="badge badge-green">✓ Konfiguriert</span>`
            : `<span class="badge badge-red">✗ Fehlt – Demo-Modus (Score 5)</span>`}
        </div>
      </div>
    </div>
  </div>
...
<script>
  function fillHost(val){
    ...
    const [h,p]=val.split(':');
    document.getElementById('acc_host').value=h;
    document.getElementById('acc_port').value=p;
  }
  async function testAccount(){
    ...
    try{
      const r=await fetch('/api/accounts/test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
...
app.post('/api/create-checkout', apiAuth, async (req, res) => {
  if (!stripe) return res.json({ ok: false, error: 'Stripe nicht konfiguriert (STRIPE_SECRET_KEY fehlt)' });
  const user = db.getUserById(req.session.userId);
  try {
...
async function start() {
  ...
  app.listen(PORT, () => {
    console.log(`🚀 LexLead v3.0 auf Port ${PORT}`);
